...
import realtimeNewsService from './realtimeNewsService.js';
...
let realtimeNewsCache = [];
const REALTIME_CACHE_SIZE = 100;

// Add this function after the imports
function initializeRealtimeNews() {
    // Subscribe to general news topics
    const newsTopics = [
        'breaking',
        'headlines',
        'top-stories',
    ...
    realtimeNewsService.subscribe(newsTopics);
    
    // Listen for news updates
    realtimeNewsService.on('news', (article) => {
        // Add to realtime cache
        realtimeNewsCache.unshift(article);
        
        // Keep cache size manageable
        if (realtimeNewsCache.length > REALTIME_CACHE_SIZE) {
            realtimeNewsCache.pop();
        }
    });
}

// Initialize realtime news when the module loads
initializeRealtimeNews();
...
async function searchHistoricalNews(params) {
    try {
        ...
        // Search across specified time ranges
        for (const timeRange of params.timeRanges) {
            promises.push(
                newsapi.v2.everything({
                    ...
                    language: 'en',
                    sortBy: 'relevancy',
                    pageSize: Math.floor(pageSize / params.timeRanges.length) // Distribute articles across time ranges
                }).catch(error => {
...
async function searchNews(params = {}) {
    try {
        ...
        // Limit the number of query variations to reduce API calls
        const enhancedQuery = preprocessQuery(params.query);
        const limitedQueryVariations = enhancedQuery.queryVariations.slice(0, 2); // Only use top 2 variations
        
        // Search for recent news with reduced article count
        const [recentNews, historicalNews] = await Promise.all([
            // Recent news (using existing sources)
            Promise.all(limitedQueryVariations.flatMap(queryVariation => [
                fetchRSSFeeds({ ...params, query: queryVariation, pageSize: 5 }),
                scrapeNewsWebsites({ ...params, query: queryVariation, pageSize: 5 }),
                newsapi.v2.everything({
                    q: queryVariation,
            ...
            // Historical news with reduced scope
            searchHistoricalNews({ 
                ...params, 
                ...enhancedQuery,
                pageSize: 10, // Limit historical results
                timeRanges: enhancedQuery.timeRanges.slice(0, 2) // Only search recent time ranges
            })
        ]);
        
        // Filter realtime news based on query (limited to most recent)
        const relevantRealtimeNews = realtimeNewsCache
            .filter(article => {
            ...
            .slice(0, 5); // Only take the 5 most recent realtime news
        
        // Combine all articles with limits
        let allArticles = [
            ...relevantRealtimeNews,
        ...
        const sortedArticles = sortArticlesByRelevanceAndRecency(uniqueArticles, params.query)
            .slice(0, 20); // Limit total articles to 20
        
        // Group articles by time period (with limits)
        const groupedArticles = {
            realtime: sortedArticles.filter(article => article.type === 'realtime').slice(0, 5),
            recent: sortedArticles.filter(article => {
                const publishDate = new Date(article.publishedAt);
                return (Date.now() - publishDate) <= 7 * 24 * 60 * 60 * 1000 && article.type !== 'realtime';
            }).slice(0, 10),
        ...
        return {
            ...
            enhancedQuery: {
                ...enhancedQuery,
                queryVariations: limitedQueryVariations // Return only used variations
            },
